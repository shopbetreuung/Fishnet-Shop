...
class billsafe_2hp {
  ...
  function update_status() {
    ...
    if (($this->enabled == true) && ((int)MODULE_PAYMENT_BILLSAFE_2HP_ZONE > 0) ) {
      $check_flag = false;
      $check_query = xtc_db_query('SELECT zone_id FROM '.TABLE_ZONES_TO_GEO_ZONES.' WHERE geo_zone_id = "'.xtc_db_input(MODULE_PAYMENT_BILLSAFE_2HP_ZONE).'" AND zone_country_id = "'.xtc_db_input($order->billing['country']['id']).'" ORDER BY zone_id');
  ...
  function selection() {
    ...
    $max_order = MODULE_PAYMENT_BILLSAFE_2HP_MAX_ORDER;
    ...
    if ($response->ack == 'OK') {
      if ($response->hirePurchase->isAvailable == 'TRUE') {
        $display = array('id' => $this->code, 'module' => $this->title, 'description' => sprintf($this->info, $response->hirePurchase->installmentCount, number_format($response->hirePurchase->installmentAmount, 2, ',', ''), number_format($response->hirePurchase->annualPercentageRate, 2, ',', '')).'<div style="clear:both;"></div>');
      } else {
        $display = array('id' => $this->code, 'module' => $this->title, 'description' => '<b><font color="#ff0000">'.$response->hirePurchase->message.'</font></b><div style="clear:both;"></div>');
//        $display = null;
      }
    } else {
      $display = null;
    ...
    if(sizeof($order->delivery) != sizeof($order->billing)) {
      ...
      if (is_array($order->billing)) {
        foreach ($order->billing as $key => $val) {
          if ($order->billing[$key] != $order->delivery[$key]) xtc_redirect(xtc_href_link(FILENAME_CHECKOUT_PAYMENT, 'payment_error='.$this->code.'&error_message='.stripslashes(urlencode(html_entity_decode(MODULE_PAYMENT_BILLSAFE_2HP_ERROR_MESSAGE_ADDRESS))), 'SSL'));
        }
      }
    }
...
//      $_SESSION['bs_article'] = $article;
  ...
  function check() {
    if (!isset($this->_check)) {
      $check_query = xtc_db_query('SELECT configuration_value FROM '.TABLE_CONFIGURATION.' WHERE configuration_key = "MODULE_PAYMENT_BILLSAFE_2HP_STATUS"');
  ...
  function install() {
    ...
    if (xtc_db_num_rows($check_query) == 0) {
      xtc_db_query('ALTER TABLE admin_access ADD billsafe_orders_2hp INT(1) NOT NULL DEFAULT "0"');
      xtc_db_query('UPDATE admin_access SET billsafe_orders_2hp = "1" WHERE customers_id = "1" OR customers_id = "groups"');
    }
    ...
    if (xtc_db_num_rows($check_query) == 0) xtc_db_query('INSERT INTO '.TABLE_CONFIGURATION.' (configuration_key, configuration_value, configuration_group_id, sort_order, date_added) VALUES ("MODULE_PAYMENT_BILLSAFE_2HP_LOG_ADDR", "", "6", "0", now())');
    $check_query = xtc_db_query('SHOW COLUMNS FROM '.TABLE_CONFIGURATION.' like "MODULE_PAYMENT_BILLSAFE_2HP_MERCHANT_ID"');
    if (xtc_db_num_rows($check_query) == 0) xtc_db_query('INSERT INTO '.TABLE_CONFIGURATION.' (configuration_key, configuration_value, configuration_group_id, sort_order, date_added) VALUES ("MODULE_PAYMENT_BILLSAFE_2HP_MERCHANT_ID", "", "6", "0", now())');
    $check_query = xtc_db_query('SHOW COLUMNS FROM '.TABLE_CONFIGURATION.' like "MODULE_PAYMENT_BILLSAFE_2HP_MERCHANT_LICENSE"');
    if (xtc_db_num_rows($check_query) == 0) xtc_db_query('INSERT INTO '.TABLE_CONFIGURATION.' (configuration_key, configuration_value, configuration_group_id, sort_order, date_added) VALUES ("MODULE_PAYMENT_BILLSAFE_2HP_MERCHANT_LICENSE", "", "6", "0", now())');
    $check_query = xtc_db_query('SHOW COLUMNS FROM '.TABLE_CONFIGURATION.' like "MODULE_PAYMENT_BILLSAFE_2HP_MIN_ORDER"');
    if (xtc_db_num_rows($check_query) == 0) xtc_db_query('INSERT INTO '.TABLE_CONFIGURATION.' (configuration_key, configuration_value, configuration_group_id, sort_order, date_added) VALUES ("MODULE_PAYMENT_BILLSAFE_2HP_MIN_ORDER", "0", "6", "0", now())');
    $check_query = xtc_db_query('SHOW COLUMNS FROM '.TABLE_CONFIGURATION.' like "MODULE_PAYMENT_BILLSAFE_2HP_MAX_ORDER"');
    ...
    if (xtc_db_num_rows($check_query) == 0) xtc_db_query('INSERT INTO '.TABLE_CONFIGURATION.' (configuration_key, configuration_value, configuration_group_id, sort_order, use_function, set_function, date_added) VALUES ("MODULE_PAYMENT_BILLSAFE_2HP_ZONE", "0", "6", "2", "xtc_get_zone_class_title", "xtc_cfg_pull_down_zone_classes(", now())');
    $check_query = xtc_db_query('SHOW COLUMNS FROM '.TABLE_CONFIGURATION.' like "MODULE_PAYMENT_BILLSAFE_2HP_ORDER_STATUS_ID"');
    if (xtc_db_num_rows($check_query) == 0) xtc_db_query('INSERT INTO '.TABLE_CONFIGURATION.' (configuration_key, configuration_value, configuration_group_id, sort_order, set_function, use_function, date_added) VALUES ("MODULE_PAYMENT_BILLSAFE_2HP_ORDER_STATUS_ID", "0", "6", "0", "xtc_cfg_pull_down_order_statuses(", "xtc_get_order_status_name", now())');
    $check_query = xtc_db_query('SHOW COLUMNS FROM '.TABLE_CONFIGURATION.' like "MODULE_PAYMENT_BILLSAFE_2HP_SORT_ORDER"');
    if (xtc_db_num_rows($check_query) == 0) xtc_db_query('INSERT INTO '.TABLE_CONFIGURATION.' (configuration_key, configuration_value, configuration_group_id, sort_order, date_added) VALUES ("MODULE_PAYMENT_BILLSAFE_2HP_SORT_ORDER", "0", "6", "0", now())');
    $check_query = xtc_db_query('SHOW COLUMNS FROM '.TABLE_CONFIGURATION.' like "MODULE_PAYMENT_BILLSAFE_2HP_ALLOWED"');
    if (xtc_db_num_rows($check_query) == 0) xtc_db_query('INSERT INTO '.TABLE_CONFIGURATION.' (configuration_key, configuration_value, configuration_group_id, sort_order, date_added) VALUES ("MODULE_PAYMENT_BILLSAFE_2HP_ALLOWED", "DE", "6", "0", now())');
  }
  ...
  function get_error_message($response) {
    foreach ($response->errorList as $error) {
      switch($error->code) {
        case '101':
          $message .= MODULE_PAYMENT_BILLSAFE_2HP_ERROR_MESSAGE_101;
          break;
        case '102':
          $message .= MODULE_PAYMENT_BILLSAFE_2HP_ERROR_MESSAGE_102;
          break;
        case '215':
          $message .= MODULE_PAYMENT_BILLSAFE_2HP_ERROR_MESSAGE_215;
          break;
        case '216':
          $message .= MODULE_PAYMENT_BILLSAFE_2HP_ERROR_MESSAGE_216;
          break;
        default:
          $message .= MODULE_PAYMENT_BILLSAFE_2HP_ERROR_MESSAGE_COMMON;
          break;
      }
    }
    return $message;
