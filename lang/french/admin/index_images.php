<?php
/* --------------------------------------------------------------
   $Id: countries.php 899 2005-04-29 02:40:57Z hhgag $   

   XT-Commerce - community made shopping
   http://www.xt-commerce.com

   Copyright (c) 2003 XT-Commerce
   --------------------------------------------------------------
   based on: 
   (c) 2000-2001 The Exchange Project  (earlier name of osCommerce)
   (c) 2002-2003 osCommerce(countries.php,v 1.8 2002/01/19); www.oscommerce.com 
   (c) 2003	 nextcommerce (countries.php,v 1.4 2003/08/14); www.nextcommerce.org

   Released under the GNU General Public License 
   --------------------------------------------------------------*/

define('HEADING_TITLE', 'Startseitenbilder');
define('TABLE_HEADING_INDEXIMAGES', 'Index Images');
define('TABLE_HEADING_SORTING', 'Sortierung');
define('TEXT_HEADING_NEW_INDEXIMAGES', 'Neues Bild');
define('TABLE_HEADING_STATUS', 'Status');
define('TABLE_HEADING_ACTION', 'Aktion');
define('TEXT_INDEXIMAGES_IMAGE', 'Bild:');
define('TEXT_INDEXIMAGES_URL', 'Bild verlinken: ');
define('TEXT_INDEXIMAGES_TITLE', 'Title f&uuml;r Bild:');
define('TEXT_INDEXIMAGES_DESCRIPTION', 'Bildbeschreibung:');
define('TEXT_INDEXIMAGES_ALT', 'Alternativer Text f&uuml;r Bild:');
define('TEXT_INDEXIMAGES_NAME', 'Name f&uuml;r Bildeintrag:');
define('SUB_INDEXIMAGE_LEFT', 'Sub-left');
define('SUB_INDEXIMAGE_RIGHT', 'Sub-right');
define('SUB_INDEXIMAGE_CENTER', 'Sub-center');
define('SUB_SUB_INDEXIMAGE_LEFT', 'Sub-sub-left');
define('SUB_SUB_INDEXIMAGE_RIGHT', 'Sub-sub-right');
define('TEXT_HEADING_EDIT_INDEXIMAGES', 'Bild bearbeiten');
define('TEXT_HEADING_DELETE_INDEXIMAGES', 'Bild l&ouml;schen');
define('TEXT_DATE_ADDED', 'hinzugef&uuml;gt am:');
define('TEXT_LAST_MODIFIED', 'letzte &Auml;nderung am:');
define('TEXT_IMAGE_NONEXISTENT', 'BILD NICHT VORHANDEN');
define('TEXT_NEW_INTRO', 'Bitte legen Sie das neue Bild mit allen relevanten Daten ein.');
define('TEXT_EDIT_INTRO', 'Bitte f&uuml;hren Sie alle notwendigen &Auml;nderungen durch');
define('TEXT_INDEXIMAGES_NAME', 'Name for image entry:');
define('TEXT_TARGET', 'Zielfenster:');
define('NONE_TARGET', 'Kein Ziel festlegen');
define('TARGET_BLANK', '_blank');
define('TARGET_TOP', '_top');
define('TARGET_SELF', '_self');
define('TARGET_PARENT', '_parent');
define('TEXT_DELETE_INTRO', 'Sind Sie sicher, dass Sie dieses Bild l&ouml;schen m&ouml;chten?');
define('TEXT_DELETE_IMAGE', 'Bild ebenfalls l&ouml;schen?');
define('ERROR_DIRECTORY_NOT_WRITEABLE', 'Fehler: Das Verzeichnis %s ist schreibgesch&uuml;tzt. Bitte korrigieren Sie die Zugriffsrechte zu diesem Verzeichnis!');
define('ERROR_DIRECTORY_DOES_NOT_EXIST', 'Fehler: Das Verzeichnis %s existiert nicht!');
define('TEXT_DISPLAY_NUMBER_OF_INDEXIMAGES', 'Anzahl Bilder in Start Index Images');
define('IMAGE_ICON_STATUS_GREEN', 'Aktiv');
define('IMAGE_ICON_STATUS_GREEN_LIGHT', 'Aktivieren');
define('IMAGE_ICON_STATUS_RED', 'Nicht aktiv');
define('IMAGE_ICON_STATUS_RED_LIGHT', 'Deaktivieren');
define('TYP_PRODUCT', 'Link zum Artikel (bitte nur die Artikel_ID bei Linkadresse eintragen)');
define('TYP_CATEGORIE', 'Link zur Kategorie (bitte nur die catID bei Linkadresse eintragen)');
define('TYP_CONTENT', 'Link zur Contentseite (bitte nur die coID bei Linkadresse eintragen)');
define('TYP_INTERN', 'Interner Shoplink (z.B. account.php oder newsletter.php)');
define('TYP_EXTERN', 'Externer Link (z.B. http://www.externerlink.com)');
define('ERROR_TEXT_HEADING', 'Fehler! Slide konnte nicht gespeichert werden:');
define('ERROR_TEXT_NAME', 'Name f&uuml;r Bildeintrag ung&uuml;ltig!');
define('TEXT_TYP', 'Linkart:');
define('TEXT_URL', 'Linkadresse:');
define('ACTIVE', 'ACTIF');
define('NOTACTIVE', 'NON ACTIF');

?>