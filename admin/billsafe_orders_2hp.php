...
*/
	...
	$order_id = $_GET['oID'];
	$post_url = 'billsafe_orders_2hp.php?oID='.$order_id;

	require ('includes/application_top.php');
	$language= $_SESSION['language'];

	require (DIR_FS_CATALOG.'lang/'.$language.'/modules/payment/billsafe_2hp.php');
	...
	$bs->setCredentials(array('merchantId' => MODULE_PAYMENT_BILLSAFE_2HP_MERCHANT_ID, 'merchantLicenseSandbox' => MODULE_PAYMENT_BILLSAFE_2HP_MERCHANT_LICENSE, 'merchantLicenseLive' => MODULE_PAYMENT_BILLSAFE_2HP_MERCHANT_LICENSE, 'applicationSignature' => $ini['applicationSignature'], 'applicationVersion' => $ini['applicationVersion']));
	...
	if(xtc_db_num_rows($orders_query) == 0) {
		$messageBox = 'ERROR';
		$message = MODULE_PAYMENT_BILLSAFE_2HP_NO_ORDERID;
	}
	...
	if (isset($_POST['reportShipmentFull'])) {
		...
		if ($responseShipment->ack == 'OK' && $full_shipment_count == 0) {
			...
			$messageBox = 'SUCCESS';
			$message = MODULE_PAYMENT_BILLSAFE_2HP_MESSAGE_FSHIPMENT;
		} else {
		...
		getBillSAFEresp($responsePartialShipment, MODULE_PAYMENT_BILLSAFE_2HP_MESSAGE_PSHIPMENT, 'reportShipmentPart', 'shipped', $orderIDArr, $order_id, $billsafe_orders['id']);
		...
		getBillSAFEresp($response, MODULE_PAYMENT_BILLSAFE_2HP_MESSAGE_PRETOURE, 'updateArticleListRetourePart', 'retoure', $orderIDArr, $order_id, $billsafe_orders['id']);
		...
		$params['order_currencyCode'] = $currency;
		$response = $bs->callMethod('updateArticleList', $params);
		getBillSAFEresp($response, MODULE_PAYMENT_BILLSAFE_2HP_MESSAGE_PSTORNO, 'updateArticleListStornoPart', 'storno', $orderIDArr, $order_id, $billsafe_orders['id']);
		...
		if (($total + $amount) >= 0) {
			...
			if ($response->ack == "OK") {
				...
				$messageBox = 'SUCCESS';
				$message = MODULE_PAYMENT_BILLSAFE_2HP_MESSAGE_VOUCHER;
			} else {
...
<div class="pageHeading col-xs-12 col-sm-6">            
    <p class="h2">
       <?php echo MODULE_PAYMENT_BILLSAFE_2HP_DETAILS; ?>
    </p>
</div>
<div class="pageHeading text-right col-xs-12 col-sm-6"><?php echo '<a class="btn btn-default" href="'.xtc_href_link(FILENAME_ORDERS, xtc_get_all_get_params(array('action'))).'">'.BUTTON_BACK.'</a>'; ?></div>
<div class="col-xs-12"><img width="100%" height="1" border="0" alt="" src="images/pixel_black.gif"></div>

<div class="col-xs-12 col-sm-4">  
<table width="100%" border="0" cellspacing="0" cellpadding="2"><tr>
<td class="main" valign="top"><strong><?php echo MODULE_PAYMENT_BILLSAFE_2HP_BADDRESS; ?></strong></td>
</tr><tr>
<td class="main"><?php
	...
	echo $customer['company'].'<br />'.$customer['firstname'].' '.$customer['lastname'].'<br />'.$customer['street'].' '.$customer['housenumber'].'<br />'.$customer['postcode'].' '.$customer['city'].'<br />'.$country['countries_name'].'<br /><br />'.MODULE_PAYMENT_BILLSAFE_2HP_EMAIL.': '.$customer['email'].'<br />';
	?>
</td></tr></table>
</div>
<div class="col-xs-12 col-sm-4"> 
<table width="100%" border="0" cellspacing="0" cellpadding="2"><tr>
<td class="main" valign="top"><strong><?php echo MODULE_PAYMENT_BILLSAFE_2HP_SADDRESS; ?></strong></td>
</tr><tr>
<td class="main"><?php
...
<div class="col-xs-12 col-sm-4"> <?php
	...
	if ($response->ack == 'OK') {
		...
		<table width="100%" border="0" cellspacing="0" cellpadding="2"><tr>
		<td class="main" valign="top"><strong><?php echo MODULE_PAYMENT_BILLSAFE_2HP_PDETAILS; ?></strong></td>
		<tr><td class="main" width="40%" valign="top"><table><tr>
			<td class="main" valign="top"><?php echo MODULE_PAYMENT_BILLSAFE_2HP_NOTE; ?></td>
			<td class="main" valign="top"><?php echo ':'; ?></td>
			<td class="main"><b><?php echo $ins->note; ?></b></td>
		</tr></table></td></tr></td></tr></table>
	<?php } ?>
</div>
<div class='col-xs-12'>
<table class='table'>
<?php echo xtc_draw_form('', 'billsafe_orders_2hp.php', 'oID='.$order_id, 'post', ''); ?>
<tr class="dataTableHeadingRow">
<td class="dataTableHeadingContent"><?php echo MODULE_PAYMENT_BILLSAFE_2HP_PRODUCTS; ?></td>
<td class="dataTableHeadingContent"><?php echo MODULE_PAYMENT_BILLSAFE_2HP_MODEL; ?></td>
<td class="dataTableHeadingContent" align="right"><?php echo MODULE_PAYMENT_BILLSAFE_2HP_TAX; ?></td>
<td class="dataTableHeadingContent" align="right"><?php echo MODULE_PAYMENT_BILLSAFE_2HP_PRICE_INC; ?></td>
<td class="dataTableHeadingContent" align="center"><?php echo MODULE_PAYMENT_BILLSAFE_2HP_CHECK; ?></td>
</tr><?php
...
<div class="col-xs-12 text-right"><?php
	if ($countOrder['quantity'] == $countUsed['quantity']) { ?>
		<input type="submit" name="reportShipmentFull" value="<?php echo MODULE_PAYMENT_BILLSAFE_2HP_FREPORT_SHIPMENT; ?>" disabled="disabled" />
		<input type="submit" name="reportShipmentPart" value="<?php echo MODULE_PAYMENT_BILLSAFE_2HP_PREPORT_SHIPMENT; ?>" disabled="disabled" />
		<input type="submit" name="updateArticleListStornoFull" value="<?php echo MODULE_PAYMENT_BILLSAFE_2HP_UPDATEARTICLELISTSTORNOFULL; ?>" disabled="disabled" />
		<input type="submit" name="updateArticleListStornoPart" value="<?php echo MODULE_PAYMENT_BILLSAFE_2HP_UPDATEARTICLELISTSTORNOPART; ?>" disabled="disabled" />
		<input type="hidden" name="id" value="<?php echo $id; ?>" />
	<?php } elseif ($countUsed['quantity'] == 0) { ?>
		<input type="submit" name="reportShipmentFull" value="<?php echo MODULE_PAYMENT_BILLSAFE_2HP_FREPORT_SHIPMENT; ?>" />
		<input type="submit" name="reportShipmentPart" value="<?php echo MODULE_PAYMENT_BILLSAFE_2HP_PREPORT_SHIPMENT; ?>">
		<input type="submit" name="updateArticleListStornoFull" value="<?php echo MODULE_PAYMENT_BILLSAFE_2HP_UPDATEARTICLELISTSTORNOFULL; ?>" />
		<input type="submit" name="updateArticleListStornoPart" value="<?php echo MODULE_PAYMENT_BILLSAFE_2HP_UPDATEARTICLELISTSTORNOPART; ?>" />
		<input type="hidden" name="id" value="<?php echo $id; ?>" />
	<?php } else { ?>
		<input type="submit" name="reportShipmentFull" value="<?php echo MODULE_PAYMENT_BILLSAFE_2HP_FREPORT_SHIPMENT; ?>" disabled="disabled" />
		<input type="submit" name="reportShipmentPart" value="<?php echo MODULE_PAYMENT_BILLSAFE_2HP_PREPORT_SHIPMENT; ?>" />
...
<br /><?php
	...
	if (xtc_db_num_rows($prod_history_query) > 0) { ?>
                <?php echo xtc_draw_form('', 'billsafe_orders_2hp.php', 'oID='.$order_id, 'post', ''); ?>
		<?php
		echo '<table border="0" width="100%" cellspacing="0" cellpadding="2"><tr class="dataTableHeadingRow"><td class="dataTableHeadingContent" align="center"><strong>'.MODULE_PAYMENT_BILLSAFE_2HP_MODEL.'</strong></td><td class="dataTableHeadingContent" align="center"><strong>'.MODULE_PAYMENT_BILLSAFE_2HP_PRODUCTS.'</strong></td><td class="dataTableHeadingContent" align="center"><strong>'.MODULE_PAYMENT_BILLSAFE_2HP_PREPORT_METHOD.'</strong></td><td class="dataTableHeadingContent" align="center"><strong>'.MODULE_PAYMENT_BILLSAFE_2HP_PREPORT_DATE.'</strong></td><td class="dataTableHeadingContent" align="center">&nbsp;</td></tr>';
		while ($prod_history = xtc_db_fetch_array($prod_history_query)) {
			if ($prod_history['transactionmethod'] == 'reportShipmentFull') {
				$method = MODULE_PAYMENT_BILLSAFE_2HP_FREPORT_SHIPMENT;
			} elseif ($prod_history['transactionmethod'] == 'reportShipmentPart') {
				$method = MODULE_PAYMENT_BILLSAFE_2HP_PREPORT_SHIPMENT;
			} elseif ($prod_history['transactionmethod'] == 'updateArticleListRetoureFull') {
				$method = MODULE_PAYMENT_BILLSAFE_2HP_UPDATEARTICLELISTRETOUREFULL;
			} elseif ($prod_history['transactionmethod'] == 'updateArticleListRetourePart') {
				$method = MODULE_PAYMENT_BILLSAFE_2HP_UPDATEARTICLELISTRETOUREPART;
			} elseif ($prod_history['transactionmethod'] == 'updateArticleListStornoFull') {
				$method = MODULE_PAYMENT_BILLSAFE_2HP_UPDATEARTICLELISTSTORNOFULL;
			} elseif ($prod_history['transactionmethod'] == 'updateArticleListStornoPart') {
				$method = MODULE_PAYMENT_BILLSAFE_2HP_UPDATEARTICLELISTSTORNOPART;
			} elseif ($prod_history['transactionmethod'] == 'updateArticleListVoucher') {
				$method = MODULE_PAYMENT_BILLSAFE_2HP_UPDATEARTICLELISTVOUCHER;
			}
		...
		if ($countOrder['quantity'] == $countNotRetoure['quantity']) { ?>
			<input type="submit" name="updateArticleListRetoureFull" value="<?php echo MODULE_PAYMENT_BILLSAFE_2HP_UPDATEARTICLELISTRETOUREFULL; ?>" />
			<input type="submit" name="updateArticleListRetourePart" value="<?php echo MODULE_PAYMENT_BILLSAFE_2HP_UPDATEARTICLELISTRETOUREPART; ?>" />
		<?php } elseif($countNotRetoure['quantity'] != 0) { ?>
			<input type="submit" name="updateArticleListRetoureFull" value="<?php echo MODULE_PAYMENT_BILLSAFE_2HP_UPDATEARTICLELISTRETOUREFULL; ?>" disabled="disabled" />
			<input type="submit" name="updateArticleListRetourePart" value="<?php echo MODULE_PAYMENT_BILLSAFE_2HP_UPDATEARTICLELISTRETOUREPART; ?>" />
		<?php } else { ?>
			<input type="submit" name="updateArticleListRetoureFull" value="<?php echo MODULE_PAYMENT_BILLSAFE_2HP_UPDATEARTICLELISTRETOUREFULL; ?>" disabled="disabled" />
			<input type="submit" name="updateArticleListRetourePart" value="<?php echo MODULE_PAYMENT_BILLSAFE_2HP_UPDATEARTICLELISTRETOUREPART; ?>" disabled="disabled" />
		<?php } ?>
		</td></form>
	<?php echo '</table>';
	} ?>
...
<table border="0" cellspacing="0" cellpadding="2"><tr class="dataTableHeadingRow">
<td class="dataTableHeadingContent"><?php echo MODULE_PAYMENT_BILLSAFE_2HP_UPDATEARTICLELISTVOUCHER; ?></td>
</tr><tr class="dataTableRow">
<td class="dataTableContent" align="right" valign="top">
<?php echo xtc_draw_form('voucher', 'billsafe_orders_2hp.php', 'oID='.$order_id, 'post', ''); ?>
<input id="voucherAmount" type="text" style="float:none;" maxlength="4" name="voucherAmount" size="4" value="" />,<input style="float:none;" id="voucherAmountKomma" type="text" maxlength="2" name="voucherAmountKomma" size="2" value="" />&nbsp;<?php echo $currency; ?>,&nbsp;<?php echo MODULE_PAYMENT_BILLSAFE_2HP_TAX.': '; echo xtc_draw_pull_down_menu('voucherTax', $tax_class_array, $tax_class_id); ?><br /><br />
<input type="submit" name="updateArticleListVoucher" value="<?php echo MODULE_PAYMENT_BILLSAFE_2HP_UPDATEARTICLELISTVOUCHER; ?>" />
</form></td></tr></table>
</div>
<div class='col-sm-3 col-xs-12'>
<a class="btn btn-default" href="Javascript:void()" onclick="window.open('<?php echo xtc_href_link('billsafe_print_order_2.php', 'oID='.$order_id); ?>', 'popup', 'toolbar=0, width=640, height=600')"><?php echo BUTTON_INVOICE; ?></a><br /><br />
<a class="btn btn-default" href="https://client.billsafe.de" target="_blank" rel="noopener"><?php echo MODULE_PAYMENT_BILLSAFE_2HP_MP; ?></a>
</div>
</div>
