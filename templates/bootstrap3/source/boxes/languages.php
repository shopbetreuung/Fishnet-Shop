...
/* -----------------------------------------------------------------------------------------
  ...
  $count_lng++;
    ...
    if (SEARCH_ENGINE_FRIENDLY_URLS == 'true') {
      $languages_string .= ' <a href="' . xtc_href_link(basename($PHP_SELF), 'language=' . $key.'&'.xtc_get_all_get_params(array('language', 'currency')), $request_type) . '">' . xtc_image('lang/' .  $value['directory'] .'/' . $value['image'], $value['name'], '', '', 'title ="'.$value['name'].'" ') . '</a> ';
    } else {
      $languages_string .= ' <a href="' . xtc_href_link(basename($PHP_SELF), 'language=' . $key.'&amp;'.xtc_get_all_get_params(array('language', 'currency')), $request_type) . '">' . xtc_image('lang/' .  $value['directory'] .'/' . $value['image'], $value['name'], '', '', 'title ="'.$value['name'].'" ') . '</a> ';
    }
