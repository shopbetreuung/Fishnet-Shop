...
// EOF - Tomcraft - 2009-10-03 - Paypal Express Modul
  ...
  if($order->info['payment_method'] == 'paypalplus'){
	  ...
	  $smarty->assign('paypal_address', xtc_address_format($order->customer['format_id'], $admin_info_array['address'], 1, '', '<br />'));
	  $smarty->assign('paypal_method', $admin_info_array['payment_method']);
	  $smarty->assign('paypal_email', $admin_info_array['email_address']);
	  $smarty->assign('paypal_account_status', $admin_info_array['account_status']);
	  $smarty->assign('paypal_intent', $admin_info_array['intent']);
	  $smarty->assign('paypal_transactions_total', format_price($admin_info_array['total'], 1, $admin_info_array['transactions'][0]['relatedResource'][0]['currency'], 0, 0));
	  $smarty->assign('paypal_transactions_state', $admin_info_array['state']);

	  //Zahlungsanweisung/Money order details
