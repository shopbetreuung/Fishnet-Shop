...
//Using this alternative makes it easier to switch between LIVE/BETA, or change your settings without touching the code.

/**
 * 2. Check the status on your order (invoice or reservation)
 */

$id = @$_GET['id']; //Your reservation or invoice number.

try {
    $result = $k->checkOrderStatus(
        $id, //Reservation, invoice number or order id.
        $type = 0 //Checks against a reservation or invoice number or if type = 1 checks against an order id.
    );
